...
```{r}
pep_data <- list(pep_oops_1, pep_oops_2, pep_oops_3, pep_oops_4) # files are part of camprotR package
...
pep_data_parsed <- pep_data_parsed %>% names() %>%
  ...
  # (these names are not consistent between samples!)
...
```{r}
psm_data <- list(psm_oops_1, psm_oops_2, psm_oops_3, psm_oops_4) # files are part of camprotR package
...
ratios_matched <- ratios %>%
  ...
  merge(all_psm_matched_data, by=c('Sequence', 'Modifications', 'replicate'))  %>%
...
# run DEqMS
fit_deqms = suppressWarnings(spectraCounteBayes(fit))
```
...
table(ifelse(DEqMS.results$sca.adj.pval<0.01, 'sig.', 'not sig.'),
      ifelse(DEqMS.results$logFC>1, 'Higher in CL', 'Lower in CL'))
