...
Quantitative proteomics using isobaric tagging such as Tandem Mass Tags (TMT) has a considerable benefit over Label-Free Quantification (LFQ) in that up to 16 samples can be quantified for each Peptide Spectrum Match (PSM). This greatly reduces the extent of missing values that may be present between different MS runs due to the limited number of ions that can be fragmented in each run and the associated issue of peptides being identified in only a subset of runs (REF).

To avoid ratio compression, quantification of TMT-labelled samples should be performed with SPS MS3 (REF). The TMT ions being quantified are therefore relatively low intensity and as such QC of the PSM-level quantification is recommended prior to summarisation to protein-level abundances.
...
We will then filter the PSMs and summarise to protein-level abundances.
...
We will want to remove low Signal:Noise (S:N) PSMs, since the quantification values will be less accurate and there will be more missing values and sub-notch values. We can inspect the relationship between S:N and missing values using the `plot_missing_SN` function.

```{r, fig.height=6, fig.width=6}
p <- plot_missing_SN(psm, bins=25)
print(p)
```
We can also look into this relationship at the tag level using `plot_missing_SN_per_sample`.
```{r}
p <- plot_missing_SN_per_sample(psm, bins=25)
print(p)
```
Based on the above, we will filter the PSMs to only retain those with S:N>10 using `filter_TMT_PSMs`. We will also remove PSMs with interference/co-isolation >50%.
```{r}
...
Now that we have inspected the PSM-level quantification and filtered the PSMs, we can summarise the PSMs to protein-level abundances. For PSM to protein summarisation, we will use `MSnbase::combineFeatures(method='robust')`. This approach can handle missing values but it only makes sense to use PSMs quantified across enough proteins and to retain proteins with enough PSMs.

Here, we will use PSMs with at most 5/10 NAs and proteins with at least three finite quantification values in a given sample. Note that this means a protein may still have a mixture of finite and NA values: If a protein has 3 PSMs and PSMs have missing values such that some samples are still quantified in both PSMs, then the protein will be quantified in these samples, but NA where missing values are present.
#### NOTE: below won't work until function below has been merged from {ts}_ptm branch into master and then into this branch.
```{r}
...
Below, we perform the summarisation. Note that robust summarisation requires log-transformed intensities. Post summarisation, we exponeniate the protein-level intensities
```{r}
