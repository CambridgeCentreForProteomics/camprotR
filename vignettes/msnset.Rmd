...
This data.frame contains have `r nrow(psm_tmt_total)` PSMs. We have 
...
In this case, it should contain a numeric matrix with `r nrow(psm_tmt_total)` rows corresponding
to the `r nrow(psm_tmt_total)` PSMs and `r sum(grepl("Abundance\\.", colnames(psm_tmt_total)))`
columns corresponding to the `r sum(grepl("Abundance\\.", colnames(psm_tmt_total)))` TMT tags.
...
This is important for extracting and combining data down the line.
...
Our quantitive data are now ready.
...
In this case, it should be a data.frame with `r nrow(psm_tmt_total)` rows corresponding
to the `r nrow(psm_tmt_total)` PSMs and any number of columns.
...
Most importantly we have to add some rownames which are identical to the 
column names of `tmt_exprs`.
...
The code below shows briefly how to save the data within an MSnSet to a csv
...
Use the `writexl` package to save to Excel.

```{r, eval=FALSE}
writexl::write_xlsx(results, path = "results.xlsx")
```

Or use the `write.csv()` function to save to csv.
