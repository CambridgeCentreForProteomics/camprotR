...
This description is a bit dense and unintelligible to the uninitiated.
...
we first need to define some terminology.
...
- This list contains multiple objects of different types:
  - The quantitative data ('assay data'; in a `matrix`)
  - The associated 'feature data' for each row in the quantification data (in a `Biobase::AnnotatedDataFrame`)
  - Metadata describing the MS experiment ('phenotype Data'; in a `Biobase::AnnotatedDataFrame`)
- These underlying objects must have a specific structure. For example the sample names in the assay data (columns) must match the sample names in the phenotype data (rownames) and the assay data and feature data must have the same number of rows.
