...
We start by reading in a file containing PSM-level output from Proteome Discoverer (PD). Here we are using a truncated file containing the first 1,000 PSMs only.
```{r}
psm_data <- read.delim(system.file("extdata", "TMT_PSMs.txt.gz", package = "camprotR"), stringsAsFactors=FALSE)
```
...
```{r}
crap_fasta_inf <- system.file("extdata", "cRAP_FullIdentifiers.fasta.gz", package = "camprotR")
...
The distribution of TMT intensities obtained by Orbitrap has been observed to have a 'notch' where few values are observed [@http://zotero.org/users/5634351/items/DLWEN74B]. This presence of the notch is dependent on the MS settings, most notably AGC and maximum injection time, and the abundance of the peptides injected. We can inspect the notch using the `plot_TMT_notch` function, which annotates the notch region and the proportion of values below the upper notch boundary. 
...
```{r, fig.height=5, fig.width=5, fig.fullwidth=TRUE, fig.cap="Missing values per PSM, in relation to the signal:noise ratio"}
p <- plot_missing_SN(psm, bins=27)
print(p)
```
...
```{r, fig.height=5, fig.width=5, fig.fullwidth=TRUE, fig.cap="Missing values per tag, in relation to the signal:noise ratio"}
p <- plot_missing_SN_per_sample(psm, bins=27)
print(p)
```
...
```{r, fig.height=3.5, fig.width=3.5, fig.fullwidth=TRUE, fig.cap="Notch plots after filtering", fig.show='hold'}
p <- plot_TMT_notch(psm) +
  ggtitle('Before filtering') +
  theme_camprot(base_size=10)
print(p)

p <- plot_TMT_notch(psm_filt_sn_int) +
