...
the FASTA database should be done carefully as it can have a 
[significant effect on protein quantification](https://pwilmart.github.io/blog/2020/09/19/shotgun-quantification-part2)
in shotgun proteomics.

Furthermore, there are certain proteinaceous contaminants that are commonly, 
...
You can download the latest version of the CCP cRAP using the `download_crap()`
...
accessions can change (slightly) over time.

```{r}
ccp_tmp <- tempfile(fileext = ".fasta")
download_crap(ccp_tmp, type = "ccp", add_crap = TRUE, verbose = TRUE)
```
...
We can see that there are `r length(mq)` sequences in cRAP.

```{r}
head(names(mq))
```

We can see that the headers for each sequence in the FASTA file have a bit
more information than the GPM cRAP headers, however they are not in a standard,
consistent format. Another reason not to use this contaminants database.

## Make your own cRAP database

You can add sequences easily enough to a FASTA database by just copying and
...
some extra commercial protease sequences added to the end.

First you would download the CCP cRAP FASTA. The following chunk is not run by
default as we have already downloaded it above.

```{r, eval=FALSE}
# download CCP cRAP
download_crap(paste0(Sys.Date(), "_CCP_cRAP.fasta"), type = "ccp")
```

Then you download the sequences you want to add into a FASTA file. 
Here we'll get some protease sequences from the bacteria _Streptomyces griseus_. 
We can download sequences from UniProt programmatically using the `httr` package.

```{r}
payload <- list(
  query = paste(c("P00776", "P00777", "P80561"), collapse = " "),
  from = "ACC+ID",
  to = "ACC",
  format = "fasta"
)
...
tmp <- tempfile()
response <- httr::GET(url = "https://www.uniprot.org/uploadlists/", 
                      query = payload, 
                      config = httr::write_disk(tmp))
```

Then we load the sequences into memory so we can adjust their headers.

```{r}
proteases <- Biostrings::readAAStringSet(tmp)
proteases
```

We want to add cRAPXXX, where XXX is a number, to the headers. We can have
a look at what the last number was in the CCP cRAP FASTA.

```{r}
tail(names(ccp_crap))
```

Using the `annotate_crap()` function we can add the cRAP numbers to the sequence
headers, starting at `cRAP128`.

```{r}
annotate_crap <- function(x, start = 1, width = 3) {
  # check input
  if (!all(grepl("\\|.*\\|", x))) {
...
names(proteases) <- annotate_crap(names(proteases), start = 128)
proteases
```

Lastly we use the `Biostrings` package to add the sequences onto the end of
our CCP cRAP database. This chunk is not run by default.

```{r, eval=FALSE}
Biostrings::writeXStringSet(
  proteases,
  filepath = paste0(Sys.Date(), "_CCP_cRAP.fasta"),
  append = TRUE
)
```
