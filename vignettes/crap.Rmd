...
`make_crap_fasta()` function. We don't want to add the `cRAP00X` numbering to 
the sequence headers just yet so we set `add_crap = FALSE`.

```{r}
griseus_tmp <- tempfile(fileext = ".fasta")
make_crap_fasta(accessions = c("P00776", "P00777", "P80561"),
                file = griseus_tmp,
                add_crap = FALSE)
```
