...
download_ccp_crap(ccp_tmp, add_crap = TRUE, verbose = TRUE)
```
...
```{r, eval=FALSE}
# download CCP cRAP
download_ccp_crap(paste0(check_uniprot_release(), "_CCP_cRAP.fasta"), type = "ccp")
```
...
response <- httr::GET(url = "https://www.uniprot.org/uploadlists/", 
                      query = payload, 
                      config = httr::write_disk(griseus_tmp))
```
...
sequences to start at 128. Finally, we use the `append_crap_fasta()` function 
from camprotR to add our _S. griseus_ sequences to our CCP cRAP FASTA.  
