...
```{r}
pep_data <- camprotR::lfq_oops_rnase
```
...
```{r}
ratios <- exprs(prot_robust[, grepl("RNase_neg", colnames(prot_robust))]) - 
  exprs(prot_robust[, grepl("RNase_pos", colnames(prot_robust))])
...
OK, so the glycoproteins are not centered at zero and there are many GO-annotated RBPs with negative log RNase -/+ ratios (2-13%).
