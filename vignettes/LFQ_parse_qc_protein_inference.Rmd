...
```{r, fig.height=4, fig.width=6, fig.fullwidth=TRUE, fig.cap="Presence/Abscence heatmap"}
...
However, there are other algorithms that are currently applied. For example MaxLFQ[@http://zotero.org/users/5634351/items/87YIDK69], which is implemented within MaxQuant, but also available via the maxLFQ function in the iq package in R. 
```{r}
...
We can now compare the protein inferences.
...
Below we define a function to plot the peptide abundances and protein inferences for the two methods for a single protein. We can ignore the details since it's the plots themselves we are interested in.
```{r}
...
```{r, fig.height=4, fig.width=5, fig.fullwidth=TRUE, fig.cap="PCA protein abundances"}
...
Finally, we consider an alternative normalisation approach. Our data is from an RNase +/- OOPS experiment. Since OOPS is known to enrich glycoproteins at the interface, we can use these proteins as an internal set of 'housekeeping' proteins which we expect to have no difference in abundance between RNase +/-.

First, we need to calculate the RNase +/- ratios, since this is the value we can normalise.
```{r}
...
ESTIMATE CV ACROSS REPLICATES AND TEST BEFORE AND AFTER NORMALISTION
```{r, fig.width=8}
plot_ratios <- function(obj){
 ...
 to_plot %>%
  ...
  to_plot %>% ggplot(aes(replicate, ratio, colour=factor(glyco_rbp))) +
  geom_quasirandom(dodge.width=2/3, size=2/3) +
...
```
Many GO-annotated RBPs with negative log RNase -/+ ratios (2-13%)

```{r}
prot_ratios[fData(prot_ratios)$glyco_rbp=='Glycoprotein',] %>% fData()

prot_ratios2 <- prot_ratios
exprs(prot_ratios2) <- 2^exprs(prot_ratios2)
glycoprotein_medians <- prot_ratios2[fData(prot_ratios2)$glyco_rbp=='Glycoprotein',] %>% camprotR::get_medians()
glycoprotein_medians
glycoprotein_medians/mean(glycoprotein_medians)
prot_ratios_norm <- camprotR::center_normalise_to_ref(prot_ratios2, glycoprotein_medians)

offset <- median(exprs(prot_ratios_norm[fData(prot_ratios_norm)$glyco_rbp=='Glycoprotein',]), na.rm=TRUE)
exprs(prot_ratios_norm) <- exprs(prot_ratios_norm)/offset
```

```{r}
plot_ratios(log(prot_ratios_norm, base=2))
```
