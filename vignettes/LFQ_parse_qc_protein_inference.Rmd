...
exprs_data <- pep_data_flt %>% 
  ...
  `colnames<-`(
    paste(
      rep(c("RNase_neg", "RNase_pos"), each = 4),
      rep(1:4, 2),
      sep = "."
    )  
  ) %>% 
...
plot_ratios <- function(obj) {
  ...
  p <- to_plot %>% ggplot(aes(Replicate, ratio, group=interaction(Glyco.RBP, Replicate), colour=factor(Glyco.RBP))) +
...
glycoprotein_medians <- prot_ratios[fData(prot_ratios)$Glyco.RBP=='Glycoprotein',] %>% camprotR::get_medians()
